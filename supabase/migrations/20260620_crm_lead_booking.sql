-- ════════════════════════════════════════════════════════════════════════
-- CRM — LEAD ↔ BOOKING/SALE link (seamless lead-centric conversion)
-- A lead remembers the reservation booked for it; get_lead derives the live
-- booking state by tracing reservation → sale_submission → sale. So the lead
-- page can show: Booked → Convert to sale → Under review → ✅ Sold.
-- ════════════════════════════════════════════════════════════════════════

ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS converted_reservation_id uuid REFERENCES public.reservations(id) ON DELETE SET NULL;

-- link_lead_reservation — stamp the reservation made for this lead (owner-scoped)
...
BEGIN
  ...
  UPDATE public.leads SET converted_reservation_id=p_reservation_id, updated_at=now()
   WHERE id=p_lead_id AND owner_sales_user_id=v_ses.sales_user_id;
  GET DIAGNOSTICS v_n = ROW_COUNT;
  IF v_n=0 THEN RETURN jsonb_build_object('success',false,'error','not_found'); END IF;
  RETURN jsonb_build_object('success',true);
END; $function$;

-- get_lead — subtree view + trail facts + BOOKING state
