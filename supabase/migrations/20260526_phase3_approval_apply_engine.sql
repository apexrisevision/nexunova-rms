-- =====================================================================
-- Phase 3 / Component 2 — PART A: Approval apply-engine.
-- Applied to itqxljtfbrppntgyfush on 2026-05-26 (migration
-- phase3_approval_apply_engine).
--
-- Extends approve_request (NOT a separate function): inside the same
-- transaction, a dispatcher reads request_type and applies payload to
-- the target entity BEFORE the status flip; any failure raises and rolls
-- the whole transaction back (request stays pending). After a successful
-- apply it writes one audit_logs row with action='approval_applied'.
-- company_id isolation is re-verified on every target entity.
--
-- Schema adaptations (columns verified against the live DB; the spec's
-- column names did not all exist):
--   * price_revision -> sales.net_amount   (no sales.total_price column;
--       net_amount is the canonical contract/total-price the app reads).
--   * dnd            -> clients.dnd_status  (no clients.is_dnd column;
--       dnd_status is the existing boolean the app/log-call modal reads).
--   * refund         -> payments.refund_amount (added below) + status='refunded'
--       (payments_status_check extended to allow 'refunded').
-- =====================================================================

-- ── Prereqs for the refund branch ────────────────────────────────────
ALTER TABLE public.payments ADD COLUMN IF NOT EXISTS refund_amount numeric;

ALTER TABLE public.payments DROP CONSTRAINT IF EXISTS payments_status_check;
ALTER TABLE public.payments ADD CONSTRAINT payments_status_check
  CHECK (status = ANY (ARRAY['received','cleared','bounced','reversed','cancelled','refunded']::text[]));

-- ── approve_request v2 (comment-enforced + admin-gated + apply dispatcher) ──
...
BEGIN
  ...
  -- ════════ APPLY PAYLOAD TO TARGET ENTITY (before status flip) ════════
  -- company_id isolation re-verified on every target row; failure raises -> tx rollback.
  CASE v_req.request_type
    ...
    WHEN 'price_revision' THEN
      UPDATE public.sales
        SET net_amount = (v_pl->>'total_price')::numeric, updated_at = now()
        WHERE id = v_req.entity_id AND company_id = v_req.company_id;
      GET DIAGNOSTICS v_rc = ROW_COUNT;
      IF v_rc = 0 THEN RAISE EXCEPTION 'entity_not_found_or_cross_company'; END IF;
  ...
  -- ════════ STATUS FLIP + COMMENT TRAIL (only reached if apply succeeded) ════════
  ...
  -- ════════ AUDIT: the approval-applied event ════════
...
EXCEPTION WHEN OTHERS THEN
  -- any apply failure rolls back the whole transaction; request stays pending
