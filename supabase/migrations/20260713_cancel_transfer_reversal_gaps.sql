-- Cancel / Transfer reversal gaps (owner-approved: payments are NOT reversed — RMS holds no
-- financials; the refund is recorded on the cancellation voucher and settled in QuickBooks).
--
-- 1. Cancellation only voided installments with status='pending'. Rows sitting at 'partial' or
--    'overdue' survived on a cancelled sale and kept showing as live dues on the unit timeline.
--    Now every not-yet-paid installment (pending/partial/overdue) is voided. 'paid' rows are left
--    untouched — they are the record of money actually received.
--
-- 2. The unit -> Available step was wrapped in `IF v_available_status_id IS NOT NULL`, so when a
--    project had no available status defined the unit silently stayed Sold while the function still
--    returned success. Now it fails loudly, before anything is written.
--
-- 4. Transfer closed the old sale (is_active=false) but left its unpaid installments at 'pending'
--    forever, so the previous owner's dues kept appearing in the unit history. Now they are voided
--    the same way a cancellation voids them.
--
-- 5. Found while testing: transfer was ALSO dead on arrival. It hand-rolled its own sale number
--    against sale_sequences(company_id, year, seq) — but that table's column is last_num, not seq,
--    so every transfer aborted with 42703 'column "seq" does not exist'. The sales table already
--    has a BEFORE INSERT trigger (trg_sale_number_gen) that assigns SAL-<year>-#### from last_num
--    when sale_number is left null, so the hand-rolled block is deleted and the trigger does it.
--
-- 6. Also found while testing: transfer inserted total_amount / net_amount / remaining_amount into
--    sales, but all three are GENERATED ALWAYS columns (price_per_sqft * area_sqft - discount
--    - down_payment). Postgres rejected it with 428C9. They are dropped from the INSERT and the
--    computed net_amount is read back with RETURNING for the transfer record and commission.
--
-- Not changed (needs an owner decision): transfer does not reverse the OLD agent's running totals
-- while the new sale accrues fresh commission, so one unit can be counted twice in agent totals.
...
DECLARE
  ...
  v_voided              INT;
BEGIN
  -- robust Available status (scoped to the unit's company+project)
  ...
  -- Refuse rather than half-cancel: without this the unit would stay Sold and the caller
  -- would still be told the cancellation succeeded.
  ...
  -- void every not-yet-paid installment (pending, partial and overdue alike).
  -- 'paid' rows stay as they are: they record money actually received.
  ...
  -- ALWAYS reverse this voided sale's contribution to the agent's running totals
...
BEGIN
  ...
  -- The old owner's schedule dies with the old sale. Without this the unpaid rows stay 'pending'
  -- and keep surfacing as live dues on the unit timeline under the previous client's name.
  UPDATE public.installments SET status='cancelled', updated_at=NOW()
  ...
  IF p_agent_id IS NOT NULL AND COALESCE(p_commission_rate,0) > 0 THEN
    INSERT INTO public.agent_transactions (
      company_id, agent_id, transaction_type, amount,
      related_sale_id, related_transfer_id,
      reference, notes, created_by
    ) VALUES (
      p_company_id, p_agent_id, 'commission_accrued',
