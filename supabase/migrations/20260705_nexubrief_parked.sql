-- ════════════════════════════════════════════════════════════════════════
-- NEXUNOVA RMS — NEXUBRIEF CONTENT FIX: parked-with-director reality  |  2026-07-05
-- ------------------------------------------------------------------------
-- "Unassigned" only counted owner=NULL pool leads. But leads that land on the
-- default receiver (a director) and never get distributed to a rep are
-- effectively unworked — the brief must say so. crm_brief_gather now reports:
--   • parked: open leads owned by a director/admin with NO activity yet
--     (not distributed to any rep), grouped by that owner
--   • untouched_top: the oldest open leads with no contact (name + hours)
--   • zero_new_streak: consecutive days (ending yesterday) with 0 new leads
-- True pool-unassigned (owner NULL) stays as its own number.
-- First names only, no phone numbers (unchanged).
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- true pool: no owner at all
  SELECT count(*) INTO v_unassigned FROM public.leads
   WHERE company_id=p_company_id AND NOT COALESCE(is_test,false)
     AND owner_sales_user_id IS NULL AND status NOT IN ('won','lost');

  -- PARKED: open leads owned by a director/admin (default receiver), no activity
  -- yet = landed and never distributed to a rep. Grouped by that owner.
  ...
  -- untouched aging: oldest OPEN leads with no contact at all (name + hours)
  ...
  v_worst := COALESCE(v_untop->0, 'null'::jsonb);   -- kept for the fallback path
  ...
  -- source drought: channel active in last 30d but 0 leads in last 3d
  ...
  -- zero-new streak: consecutive days ending yesterday with no new leads
  v_streak := 0;
...
-- ════════════════════════════════════════════════════════════════════════
-- DEPLOY DEPS: redeploy edge fn `crm-daily-brief` (sharper prompt + parked-aware
-- fallback). DB migration applied = gather live.
-- ════════════════════════════════════════════════════════════════════════
