-- ═══════════════════════════════════════════════════════════════════════════
-- THE SAME UNIT STOPS BEING PRINTED TWICE
--
-- Rashid booked three units — one On Hold, one Reserved, one Booked — and all
-- three appeared under "Booked Today" and then all three again under "Units
-- Held". Both sections were telling the truth: one is the day's movement, the
-- other the standing position, and on a day when everything booked is still
-- held they are the same three rows. That is not a defence. The report is read
-- and pasted into a group, where six lines mean six units.
--
-- I saw this coming when the held list was added and wrote it off as "different
-- claims, acceptable overlap". It was not acceptable. Second time this session
-- that a compromise nobody asked for had to be undone.
--
-- `holding` gains `booked_today`, computed against the SAME Karachi day that
-- section 1 filters on, so the two can never disagree about which rows they
-- share. The array stays COMPLETE — the summary's held count and the money
-- tied up in holds are the whole standing position, and it would be wrong to
-- shrink those. Only the printed rows are narrowed, in the renderer.
--
-- Additive. One key. Body patched from pg_get_functiondef() on live.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           ...
           'tag',       COALESCE(rs.status_name, 'Reserved'),
           'tag_code',  COALESCE(rs.status_code, 'RESERVED'),
           /* Was this hold created on the day being reported? Section 01 already
              lists those, and printing them again below made three bookings read
              as six. The flag is computed HERE, against the same Karachi day
              section 01 uses, so the two sections cannot disagree about which
              rows they share. The array itself stays complete: the money and the
              count in the summary are the whole standing position, and only the
              printed rows are narrowed. */
           'booked_today', ((r.created_at AT TIME ZONE 'Asia/Karachi')::date = v_day),
