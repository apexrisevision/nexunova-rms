-- The same sequencing problem as 20260828u, one layer up.
--
-- The live page still sends p_*_url. 20260828s changed this function to demand
-- p_*_path, so from the moment it was applied every real registration would
-- have answered "Please add your photo and both sides of your CNIC" to somebody
-- who had just added exactly that. A correct rule enforced before the thing it
-- governs exists is not correctness; it is an outage.
--
-- So it takes either. A path is preferred and checked against the token in the
-- usual way. A URL is accepted as it always was, and stored where it always
-- was. The URL branch is transitional and comes out once the new page has been
-- serving long enough that no browser is still holding the old one.
...
DECLARE v_co public.companies; v_pending int; v_norm text; v_existing text; v_cnic text; v_uid uuid; v_umb boolean := false;
        ...
        v_upp text; v_ucf text; v_ucb text;
BEGIN
  ...
  v_upp := NULLIF(TRIM(COALESCE(p_profile_photo_url,'')),'');
  v_ucf := NULLIF(TRIM(COALESCE(p_cnic_front_url,'')),'');
  v_ucb := NULLIF(TRIM(COALESCE(p_cnic_back_url,'')),'');

  IF v_pp IS NOT NULL AND v_cf IS NOT NULL AND v_cb IS NOT NULL THEN
    -- the new page. A path is a claim about where a file is, and a claim made by
    -- a browser is checked, not believed: it must be under this token's folder.
    ...
    END IF;
    v_upp := NULL; v_ucf := NULL; v_ucb := NULL;
  ELSIF v_upp IS NOT NULL AND v_ucf IS NOT NULL AND v_ucb IS NOT NULL THEN
    -- a page loaded before this pass shipped. Accepted exactly as before.
    v_pp := NULL; v_cf := NULL; v_cb := NULL;
  ELSE
