-- ════════════════════════════════════════════════════════════
-- PROJECT-SCOPING — BATCH 6 GROUP 6B: server-side isolation on sale list/detail RPCs
-- 2026-05-30.  ⚠️ REVIEW ONLY — DO NOT APPLY until approved.
-- ════════════════════════════════════════════════════════════
-- Retrofits 14 sale list/detail RPCs with the standard pattern:
--   _rms_caller() → v_all = (no session) OR _rms_is_admin → else v_pids = assigned projects
--   filter: AND (v_all OR s.project_id = ANY(v_pids))
-- Anon (no session) keeps the PERMISSIVE default (v_all = true).
--
-- HARD LINE preserved: the 9 report RPCs and buyer-portal RPCs are NOT touched.
--   list_sales_for_report                ← report RPC, stays caller-blind
--   get_buyer_sale_summary               ← buyer-portal token-keyed, stays untouched
-- Already gated in prior batches (not re-touched here):
--   list_sales_by_agent       (Batch 4 Group 4C)
--   list_sales_by_client      (Batch 3 Group 3C)
--   list_sales_by_client_all  (Batch 3 Group 3C)
--   get_unit_sales_count      (Batch 6 Group 6A)
-- Payment/installment-keyed dependents → Group 6C / 6D (not this group).
--
-- ────────────────────────────────────────────────────────────
-- LISTS (6) — direct s.project_id gate
-- ────────────────────────────────────────────────────────────
...
-- get_sales_register already accepts p_project_id; extend with v_pids gate.
-- When p_project_id is supplied and is NOT in the caller's allowed set, return empty.
-- Otherwise filter rows by s.project_id ∈ v_pids (admin/anon see everything).
...
AS $function$
  ...
  FROM (
    ...
    WHERE s.company_id = p_company_id
      ...
      AND (p_project_id IS NULL OR cfg.v_all OR p_project_id = ANY(cfg.v_pids))
  ) q;
$function$;
...
-- ────────────────────────────────────────────────────────────
-- DETAIL BY ID (8) — direct s.project_id gate
-- ────────────────────────────────────────────────────────────
...
-- get_sale_documents_amendments: early-gate on parent sale.project_id.
-- If the caller cannot see the parent sale, return an empty docs/amendments envelope
-- (same shape as the "no documents" state — never leak existence of the sale).
