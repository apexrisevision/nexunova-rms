-- Portfolio Summary report RPC.
-- Reuses get_recovery_position as the ACTIVE spine (so net/received tie to Recovery
-- Position exactly), and enriches with agent name + status. Cancelled rows are pulled
-- directly from sales (received = sum of received payments) for the All/Cancelled views.
...
BEGIN
  ...
  active_src AS (
    SELECT e.sale_id, e.unit_no, e.floor_name, e.area, e.unit_rate,
           e.total_price, e.discount, e.net_price, e.received_total
...
GRANT EXECUTE ON FUNCTION public.get_portfolio_summary(uuid, uuid, date, text) TO anon, authenticated, service_role;
