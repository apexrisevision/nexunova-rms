-- list_my_leads: expose owner_role so the UI can build a director's distribute pool.
-- 2026-06-21. The "Distribute leads" screen filtered to is_mine, so a director
-- whose incoming (lead_entry) leads are owned by ANOTHER director saw nothing to
-- hand down. Adding owner_role lets the client show the director-level pool
-- (owner is a director = not yet pushed to a manager/rep). Company-wide scope for
-- director/admin/cfo is unchanged (see 20260621_directors_see_all_company_leads).
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'owner_name', ow.full_name, 'owner_sales_user_id', l.owner_sales_user_id, 'owner_role', ow.role,
