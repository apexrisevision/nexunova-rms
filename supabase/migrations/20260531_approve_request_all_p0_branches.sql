-- P0: approve_request — consolidated apply-branches for all gated request types.
-- Admin-only; applies each from request payload via the locked _core executors; audits.
-- Supersedes 20260531_approve_request_payment_void_branch.sql (which added only payment_void).
...
DECLARE
  ...
  v_pl jsonb; v_rc integer; v_res jsonb;
BEGIN
  ...
  VALUES (v_req.company_id, v_req.entity_table, v_req.entity_id::text, 'approval_applied', v_pl, v_me.id, v_me.full_name, v_me.role, true, 'approvals', v_req.request_type);
  RETURN jsonb_build_object('success',true,'status','approved','entity_table',v_req.entity_table,'entity_id',v_req.entity_id,'applied',true);
