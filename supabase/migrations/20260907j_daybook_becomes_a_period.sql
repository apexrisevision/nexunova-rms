-- ═══════════════════════════════════════════════════════════════════════════
-- THE DAYBOOK BECOMES A PERIOD REPORT
--
-- Rashid: a From and a To, with an opening balance, the movement between them
-- and a closing balance; nothing outside the range may appear; and the unit
-- lists sorted the way a building is walked — Lower Ground, Ground, First,
-- Second — rather than by the clock.
--
-- THE ONE RULE EVERYTHING RESTS ON: a reservation is HELD at instant T when it
-- was created before T, was not cancelled before T, had not expired by T, and
-- had not become a sale by T. The opening balance is that rule at the start of
-- the period, the closing balance is the same rule at the end, and the printed
-- rows are the same rule again. They cannot drift, because _daybook_held is one
-- function called three times rather than three queries that agree today.
--
-- REMOVED IS NOT COUNTED, IT IS DERIVED: opening + added - closing. A released
-- row that some query forgot then shows up as an arithmetic error on the page
-- instead of quietly vanishing.
--
-- The floor table is built from the same predicates rather than from
-- units.status_id, because that column has no history and a report for last
-- week would otherwise be drawn with this week's board. `other` is the single
-- figure that cannot follow: a unit marked Dead carries no date, so it counts
-- as Dead throughout. It renders only when non-zero.
--
-- p_date still works and still means a one-day period, so a client that has
-- not been redeployed behaves exactly as before.
--
-- The guard block — session, lead_entry, _sales_may_sell, umbrella span, scope,
-- _sales_sees_prices — is taken verbatim from pg_get_functiondef() on live.
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

CREATE OR REPLACE FUNCTION public._daybook_held(
  p_companies uuid[], p_scope uuid, p_at timestamptz, p_at_date date)
 RETURNS TABLE (
   unit_id uuid, unit_no text, floor text, rank int, num int,
   area numeric, area_unit text, base_price numeric,
   requested_by text, agent_code text, booked_by text, client_name text,
   token_amount numeric, tag text, tag_code text,
   created_at timestamptz, expiry_date timestamptz)
 LANGUAGE sql
...
REVOKE ALL ON FUNCTION public._daybook_held(uuid[], uuid, timestamptz, date) FROM PUBLIC;
-- Internal. get_reservation_daybook is SECURITY DEFINER, so inside it the
-- current user is the owner and this needs no grant of its own. CREATE FUNCTION
-- hands EXECUTE to PUBLIC by default, which is the hole that has to be closed
-- on every new function rather than assumed shut.
...
/* ── GRANTS AND THE OLD SIGNATURES ─────────────────────────────────────────
   CREATE OR REPLACE matches on name AND argument types, so adding p_from and
   p_to did not replace the three-argument daybook — it created a second one
   beside it. That already happened once tonight and went unnoticed:
   reserve_unit_desk has been live in TWO versions since the tag work, the old
   eleven-argument one that hardcodes Reserved and records no tag, and the new
   twelve. A caller sending only the original eleven names still reaches the old
   one. Both old signatures go.

   Dropping them is safe for a browser that has not been redeployed. PostgREST
   calls by NAME, so a call naming only the original arguments resolves against
   the new signature with the added ones taking their defaults — which is exactly
   the old behaviour.

   And the grants. CREATE FUNCTION hands EXECUTE to PUBLIC by default, which is
   why the twelve-argument reserve_unit_desk carries =X/postgres in its ACL while
   every other desk RPC does not. CREATE OR REPLACE keeps whatever ACL a function
   already had, so only genuinely NEW functions are exposed this way — and both
   of tonight's are. They are set to match the others exactly: no PUBLIC, and
   anon plus authenticated, because the portal runs on the anon key and every one
   of these is gated on a session token rather than on the role. */
DROP FUNCTION IF EXISTS public.get_reservation_daybook(text, date, uuid);
DROP FUNCTION IF EXISTS public.reserve_unit_desk(text, uuid, uuid, uuid, text, text, text, integer, boolean, numeric, text);

REVOKE ALL ON FUNCTION public.get_reservation_daybook(text, date, uuid, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_reservation_daybook(text, date, uuid, date, date) TO anon, authenticated, service_role;

REVOKE ALL ON FUNCTION public.reserve_unit_desk(text, uuid, uuid, uuid, text, text, text, integer, boolean, numeric, text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reserve_unit_desk(text, uuid, uuid, uuid, text, text, text, integer, boolean, numeric, text, uuid) TO anon, authenticated, service_role;

COMMIT;

-- ── applied immediately after, once the ACL was read back ──────────────────
-- The REVOKE above named only PUBLIC, and the schema's default privileges had
-- already handed EXECUTE to `authenticated`. _daybook_held takes a company
-- array and a scope directly and performs NO session check of its own — it is
-- safe only because the one function that calls it has already established
-- both. Left as it was, any signed-in user could pass another tenant's company
-- id and read its reservations. get_reservation_daybook is SECURITY DEFINER
-- and owned by postgres, so it can still call this with no grant at all.
REVOKE ALL ON FUNCTION public._daybook_held(uuid[], uuid, timestamptz, date)
