-- ============================================================================
-- NEXUNOVA RMS — store nominee CNIC images on the client (parity with the
-- client's own CNIC images + the agent's KYC docs). 2026-06-17.
-- Adds clients.next_of_kin_cnic_front_url / next_of_kin_cnic_back_url and wires
-- them through create_client + _update_client_core so they save with the client
-- (and therefore with the sale that links to it).
-- ============================================================================
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS next_of_kin_cnic_front_url text;
ALTER TABLE public.clients ADD COLUMN IF NOT EXISTS next_of_kin_cnic_back_url  text;
...
BEGIN
  ...
  INSERT INTO public.clients (
    ...
    next_of_kin_cnic, next_of_kin_photo_url, next_of_kin_cnic_front_url, next_of_kin_cnic_back_url,
    ...
    NULLIF(p_data->>'next_of_kin_cnic_front_url',''), NULLIF(p_data->>'next_of_kin_cnic_back_url',''),
...
-- _update_client_core: handle the two new keys (present-key-only update)
...
BEGIN
  ...
  UPDATE public.clients SET
    ...
    next_of_kin_cnic_front_url = CASE WHEN p_data ? 'next_of_kin_cnic_front_url' THEN NULLIF(p_data->>'next_of_kin_cnic_front_url','') ELSE next_of_kin_cnic_front_url END,
    next_of_kin_cnic_back_url  = CASE WHEN p_data ? 'next_of_kin_cnic_back_url'  THEN NULLIF(p_data->>'next_of_kin_cnic_back_url','')  ELSE next_of_kin_cnic_back_url  END,
