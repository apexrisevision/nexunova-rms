...
--
-- Two things stood between Rashid and releasing a reservation early.
--
-- cancel_reservation would only cancel a hold you had booked YOURSELF
-- (reserved_by = your own user). That is right for a rep and wrong for the
-- person who approves the queue: he books units for other people all day, so
-- letting one go for them is the same job. A director may now release any
-- active hold in his own company — his own or anybody's — and everyone else
-- keeps exactly the rule they had.
--
-- And the daybook's standing hold list named a unit on every row and never
-- named the hold, so there was nothing to act on even once he was allowed to.
-- _daybook_held now returns the reservation id and the daybook passes it into
-- both hold lists.
--
-- Additive on both counts: no existing caller loses anything, and a row that
-- gains a field breaks nothing that was reading the others.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- reserved_by uniquely identifies the dealer's own reservation (any member company)
  /* WHOSE HOLD IS IT TO RELEASE?
     Yours, always. And a director's, over anything in their own company —
     which is the whole point of the request queue: he approves a hold for
     somebody else, so he has to be able to let it go for them too, and
     before the clock runs out rather than only after. Until now the only
     way back was the Reservations screen in RMS, and only for a hold he
     had booked himself.

     Still one company. A director is not a director of everybody. */
  SELECT * INTO v_res FROM public.reservations WHERE id=p_reservation_id
     AND status='active'
     AND (reserved_by = v_ses.sales_user_id
          OR (company_id = v_ses.company_id
              AND COALESCE(public._sales_role_of(p_session_token),'') = 'director'));
...
/* The signature changes, and CREATE OR REPLACE cannot change a return type,
   so this one is dropped and rebuilt. get_reservation_daybook calls it by
   name and is recreated below in the same transaction, so nothing is ever
   left pointing at a function that is not there. */
DROP FUNCTION IF EXISTS public._daybook_held(uuid[], uuid, timestamptz, date);
...
/* A DROP takes the old grants with it and a fresh CREATE hands EXECUTE to
   PUBLIC. This helper takes a company array and checks no session at all,
   so it has to stay reachable only from the functions that scope it. */
REVOKE ALL ON FUNCTION public._daybook_held(uuid[], uuid, timestamptz, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._daybook_held(uuid[], uuid, timestamptz, date) FROM anon, authenticated;
