-- Phase 2 — follow-up: the leads LIST is deal-sourced, not lead-sourced.
--
-- Found by the real-browser run, not by reading the code: the Leads tab renders
-- from list_my_deals (P4 made the pipeline deal-sourced), so the is_locked flag
-- added to list_my_leads never reached the list and no lock chip appeared.
-- Original dumped to migration_work/provenance/phase2_followup_rpcs_before_20260813.sql
-- Additions marked -- PHASE2; nothing else changed.
