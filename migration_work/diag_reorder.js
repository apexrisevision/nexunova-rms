...
const ROOT=path.resolve(__dirname,'..');const PORT=4502;const BASE=`http://127.0.0.1:${PORT}`;
...
const diag=await page.evaluate(async()=>{
  ...
  const r1=await saveFloor({id:gid, sort_order:9});                 // no company_id (reorder path)
  const r2=await saveFloor({id:gid, company_id:scid, sort_order:9}); // with company_id
