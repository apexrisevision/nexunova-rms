...
const ROOT=path.resolve(__dirname,'..');const PORT=4723;const BASE=`http://127.0.0.1:${PORT}`;
...
(async()=>{const srv=await serve();
  ...
  await page.evaluate(()=>{document.getElementById('s-onboarding')?.classList.remove('on');nav('units');});await sleep(3500);
  const hdr=await page.evaluate(()=>{const sub=Array.prototype.map.call(document.querySelectorAll('#pg-units *'),e=>e.childNodes.length===1?e.textContent.trim():'').find(t=>/\bUNITS\b/i.test(t)&&t.length<60)||'';return sub;});
