/** Sale Detail page screenshot (live ZZTEST) — full page, 1366 light+dark. */
const puppeteer = require('puppeteer-core');
const http = require('http'); const path = require('path'); const fs = require('fs');
const ROOT = path.resolve(__dirname, '..');
const PORT = 4795; const BASE = `http://127.0.0.1:${PORT}`;
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const OUT = path.join(__dirname, 'saledetail_shots');
...
(async()=>{
  ...
  await page.evaluate((id)=>openSaleDetail(id), SALE);
  try { await page.waitForFunction(()=>{ const c=document.querySelector('#pg-salesdetail .card'); return c && /ZZTEST|SAL-/.test(document.querySelector('#pg-salesdetail')?.textContent||''); }, { timeout:12000 }); } catch(e){ console.log('wait timeout'); }
  await sleep(1200);

  for (const theme of ['dark','light']){
    await page.evaluate(t=>document.documentElement.setAttribute('data-theme',t),theme); await sleep(500);
    await page.mouse.move(0,0); await sleep(60);
    await page.screenshot({path:path.join(OUT,`saledetail_${TAG}_${theme}.png`), clip:{x:236,y:44,width:1130,height:640}});
    console.log('  shot', TAG, theme);
  }
  // scrolled viewport shot (schedule / lower sections), light theme
  await page.evaluate(t=>document.documentElement.setAttribute('data-theme',t),'light'); await sleep(300);
  await page.evaluate(()=>window.scrollTo(0,820)); await sleep(500);
  await page.screenshot({path:path.join(OUT,`saledetail_${TAG}_scroll.png`)});
  console.log('  shot', TAG, 'scroll');
