...
const PORT = 4803; const BASE = `http://127.0.0.1:${PORT}`;
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const OUT = path.join(__dirname, 'userform_shots');
...
(async()=>{
  ...
  const opened = await page.evaluate(()=>{ try{ openAddUserModal(); return 'ok'; }catch(e){ return 'ERR:'+e.message; } });
  console.log('OPEN', opened); await sleep(1500);
  const m1 = await page.evaluate(()=>{ const ov=document.querySelector('#um-modal-host .nx-modal-overlay'); const mo=document.querySelector('#um-modal-host .nx-modal'); const bd=document.querySelector('#um-modal-host .nx-modal-body'); const ti=document.querySelector('#um-modal-host .nx-modal-title'); const tr=ti?ti.getBoundingClientRect():null; const mr=mo?mo.getBoundingClientRect():null; const cs=bd?getComputedStyle(bd):null;
    return { open:!!ov, fields:document.querySelectorAll('#um-modal-host .nx-input,#um-modal-host .nx-select').length, title:(ti?.textContent||''), titleTop: tr?Math.round(tr.top):null, modalTop: mr?Math.round(mr.top):null, modalBot: mr?Math.round(mr.bottom):null, modalH: mr?Math.round(mr.height):0, vh: window.innerHeight,
  ...
  const chain = await page.evaluate(()=>{ let el=document.querySelector('#um-modal-host .nx-modal-overlay'); const out=[]; while(el && el!==document.documentElement){ const cs=getComputedStyle(el); if(cs.transform!=='none'||cs.filter!=='none'||cs.willChange!=='auto'||cs.perspective!=='none'||(cs.contain&&cs.contain!=='none'&&cs.contain!=='normal')) out.push({tag:el.tagName.toLowerCase(),id:el.id||'',cls:(el.className||'').toString().slice(0,40),transform:cs.transform.slice(0,30),filter:cs.filter.slice(0,20),willChange:cs.willChange,contain:cs.contain}); el=el.parentElement; } return out; });
  ...
  await page.evaluate(()=>{ try{ saveUserModal(); }catch(e){} }); await sleep(800);
  const fb = await page.evaluate(()=>{ const e=document.getElementById('um-err'); return { errShown: e && e.style.display!=='none' && e.offsetParent!==null, errText:(e?e.textContent:'').slice(0,80), errInView: e?(()=>{const r=e.getBoundingClientRect();return r.top>=0&&r.bottom<=window.innerHeight;})():false }; });
