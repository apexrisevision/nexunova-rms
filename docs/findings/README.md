...
| [2026-09-16-O](2026-09-16-O-backup-dies-silently-and-looks-successful.md) | `backup-full.js` can die halfway and look like it succeeded | `scripts/backup-full.js` — **every backup, all tenants** | **FIXED 2026-09-16.** Killed mid-run on 16 Sep: exit 0, no manifest, 163 of 173 tables, and nothing said so. Now streamed to disk, counts checked against live, manifest written last, `DONE` marker, `--verify`. Proved by killing it on purpose. |
