...
- ✅ **Pre-launch session (2026-06-01) — completed:**
  - Finance role verified in-browser (accounts@ADMIN): Finance nav group wakes, System/origination hidden, no read-only badge — correct.
  - Manager role bug FOUND + FIXED (commit 86d70dc): manager was locked out of every page ("grant module access") because hasPermission() required populated module_permissions but create_app_user leaves {}. Added role-based read fallback in helpers.js (manager gets broad READ like recovery/accounts; writes still blocked by role-readonly + server authz) + footer label fix. Verified in-browser. This would have locked out EVERY real manager at launch.
  - Recovery Queue audited clean (no issues).

- ⏸️ **WhatsApp DEFERRED from tomorrow's launch (2026-06-01):** WABA 2055726832037039 is still in TEST mode → payment configuration is locked ("You currently can't access payment configurations — contact Meta Business Engineering"), payment country stuck on "India" (cannot self-serve change in Test mode), prod number +92 320 1983809 + Green Tick still blocked. Root cause: Meta business verification (NexuNova SECP docs) not yet cleared — this is a Meta-side review, not a code/config fix. DECISION: launch core RMS (sales/payments/recovery — production-ready) WITHOUT WhatsApp; enable WhatsApp after Meta business verification clears. Action to unblock: submit NexuNova SECP docs to Meta Business Verification (Security Center) — start ASAP, it's a multi-day review.

- 🔲 **Remaining launch-prep TODO (for tomorrow):**
  1. Raise plan user limit 4 → 8 (launch will have 7-8 users; ADMIN co is on Pro 4-cap).
  2. Clean up 2 test users (accounts@ADMIN, manager2@ADMIN) + reactivate the 2 deactivated originals: UPDATE app_users SET status='active' WHERE id IN ('18800ace-1dcd-4a2b-80e8-d43fe96578df','49541eee-f317-47b2-84ed-3c665421c9eb') — but deactivate/remove the 2 test users FIRST (4-cap), OR do this after step 1 raises the cap.
  3. Fresh pre-launch DB backup (6 authz migrations + manager fix shipped today).
  4. Launch-day smoke test: admin + officer login, one sale, one payment, one report — end-to-end on live.
...
> **Build order (historical):** all 5 phases are built ✅ (see the per-phase COMPLETE lines above). The original "do not jump ahead — each phase depends on the previous" sequencing rule applied **during** the build and is retained only as context; it no longer implies in-progress work. Current work is **post-build hardening, demand-driven** — no phase is "in progress" or "next."
