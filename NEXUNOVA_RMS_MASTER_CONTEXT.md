...
**Admin-visibility consent (2026-05-29 refinement — overrides the owner/super-admin bypass for VISIBILITY only):** In this package there is **no "owner" concept** — a company runs as **1 Admin + 3 users**, and the Admin is a *temporary employee*, not the data owner. So a user's dashboard DATA and ACTIVITY are shown to the Admin **only after that user one-time accepts** (forced on first login; revocable from the user menu → "Data sharing"; strictly per-user). Until granted (`pending`) or if `declined`, that user's activity is masked and any project they solely steward is hidden from the Admin. This **refines the bypass rule above**: the owner/super-admin bypass still applies to **authority**, but **visibility is now consent-gated even for the Admin**. **Authority is NEVER gated** — the Admin always creates/deletes users, restores passwords, assigns rights, and manages site assignments regardless of consent. Enforced in: the dashboard `cc_*` functions **and** all 9 report RPCs (`get_collection_report`, `get_sales_register`, `get_outstanding_report`, `get_unit_inventory`, `get_aging_report`, `get_project_summary`, `get_tax_wht_report`, `get_post_possession_dues_report`, `get_legal_portfolio`), with a "**X site(s) hidden — consent pending**" note on `reports/viewer.html`. Non-admin report callers are scoped to their assigned sites only (no consent hide — consent never hides a user's own data from themselves; a non-admin with no assignment sees nothing until assigned). Table `user_admin_consent`; helpers `_cc_hidden_project_ids` / `_cc_is_admin_view` / `_cc_caller_project_ids`; session RPCs `get_my_consent_status` / `set_my_consent`. Migrations: `20260529_admin_consent.sql` + `20260529_reports_consent.sql`.
...
- ✅ **Admin-visibility CONSENT layer (2026-05-29)** — visibility (dashboard + activity + site data) is now consent-gated **even for the Admin** (see §3 refinement). No owner concept in the package — **1 Admin + 3 users**, Admin is a temporary employee. **Authority is never gated** (user create/delete, password restore, rights, site assignment always work). New table `user_admin_consent` (RLS deny_all + audit trigger; "no row = pending"); session RPCs `get_my_consent_status` / `set_my_consent`; helpers `_cc_hidden_project_ids` / `_cc_is_admin_view` / `_cc_caller_project_ids`. Forced consent modal on login for non-admins (`js/auth.js` + re-prompt on restore in `js/init.js`), revocable via sidebar user menu → "Data sharing". **Gated surfaces:** (a) dashboard `cc_command_center` / `cc_team_activity` / `cc_user_contacts` — Command Center masks non-granted users (`js/pages/dashboard.js` + `css/dashboard-premium.css`) and hides non-consented sites; (b) **all 9 report RPCs** (Collection, Sales Register, Outstanding, Unit Inventory, Aging, Project Summary, Tax/WHT, Post-Possession, Legal) — admin/anon view hides non-consented projects, **non-admin scoped to assigned sites only**. `reports/viewer.html` hydrates the app's shared Supabase session (same-origin `localStorage`) and shows a "**X site(s) hidden — consent pending**" amber note. **Electron verified:** the desktop wrapper opens reports via `window.open` in a new **in-app BrowserWindow** (same default session, same origin) — NOT the OS browser (`electron/main.js` has no `shell.openExternal`/`setWindowOpenHandler`), so `auth.uid()` resolves and the gate works in the desktop build (confirmed with an Electron 35.7.5 harness). Migrations: `20260529_admin_consent.sql` (Command Center layer — committed `b1e810f`) + `20260529_reports_consent.sql` (reports gate).
