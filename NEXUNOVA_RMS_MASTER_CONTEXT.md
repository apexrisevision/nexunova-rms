...
- ✅ **Recovery Queue module audit + 11 bugs fixed (2026-05-28)** — migration `20260528_recovery_fixes.sql`: `list_broken_promises` upgraded with `p_project_ids uuid[] DEFAULT NULL` isolation param (recovery officers see only their assigned projects; admin/owner pass NULL → all). 11 bugs fixed across `fieldvisits.js` + `contacts.js`: (1) **🔴 Critical:** `fvSyncOfflineQueue` direct `.from('contact_logs').insert()` → `create_contact_log` RPC (was blocked by deny_all_anon RLS, offline sync queue permanently stuck); (2) **🟡 Isolation:** `list_broken_promises` now project-scoped in both RPC + calling code; (3) **🟠 9 silent-null data bugs** in `_fcEscalation` / `_fcQueue` — `p.promised_date→p.promise_date`, escalations rows now use real columns (`created_at`, `from_level`, `to_level`, `reason`, `status`, `escalated_to`), legal cases now use `filed_date`+`stage` colour map (not `filing_date`+`status`), removed phantom `court_name`/`forum` column references.

**Immediate next action:** **Phases 1–5 complete + Module 7 dispatch built + Auth Chapter complete + Recovery Queue bugs fixed (2026-05-28).** Comms go-live blocked on provider creds. Auth email go-live blocked on SMTP secrets (or Resend key). Remaining open: Module 7 SMS/email adapter cases (small) + email-provider decision; deliberately-deferred core-flow items (Module 4 allocation/overpayment) and `[s]` skips; the Next.js + React rebuild (Tremor + shadcn, full UI v2) is the open future direction (see §1 / §8).
