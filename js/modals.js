...
function openUnitModal(unitId){
  ...
  const projSel=document.getElementById('un-proj');
  if(projSel){
    const prjs=(gdb().projects?.[S.cid])||[];
    projSel.innerHTML='<option value="">No project (unassigned)</option>'+prjs.map(p=>`<option value="${p.id}"${u?.projectId===p.id?' selected':''}>${esc(p.name)}</option>`).join('');
  }
  om('m-unit');
}
function saveUnit(){
  ...
  if(_eunid){
    ...
    if(idx>=0){
      ...
      units[idx].projectId=document.getElementById('un-proj')?.value||units[idx].projectId||'';
    ...
    units.push({id:'u'+uid(),companyId:S.cid,projectId:document.getElementById('un-proj')?.value||'',unitNo:no,floor:fl,floorLabel:flM[fl]||fl,type:newType,area:newArea,status:newStatus,customerName:'',phone:'',bookingNo:'',soldBy:'',soldDate:'',totalPrice:0,totalPaid:0,pendingAmount:0,lastPaymentDate:'',lastContactDate:'',remarks:document.getElementById('un-rem')?.value?.trim()||''});
  }
