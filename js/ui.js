...
const MODULE_NAV_ITEMS = [
  ...
  { key:'units',     id:'unitmap',    ic:'map',         lb:'Unit Map' },
  ...
  if(isA){
    navGroups = [
      ...
      { label: 'Sales & Money', items: [
        ...
        // Daily Closing sits with the money, next to Record Payment, because
        // that is the drawer it reconciles. DEFAULT-CLOSED: the item is only
        // built when the server has actually sent daily_closing === true, so a
        // tenant without the module gets a byte-identical sidebar to the one
        // it had before this line existed. nav() refuses the page as well.
        (window._featureFlags && window._featureFlags.daily_closing === true)
          ? [{ id:'dailyclosing', ic:'wallet', lb:'Daily Closing' }] : []
      )},
      ...
      { label: 'Recovery', items: [
        ...
        { id:'callsheet',   ic:'clipboard-list', lb:'Call Sheet' },
...
function nav(pg,x){
  ...
  // ⚠️ Daily Closing is DEFAULT-CLOSED, unlike every flag above it.
  // hasFeature() returns TRUE for a key it has never seen — the SaaS model is
  // deliberately default-open — which would have put the cash book into
  // Khushal Bagh's and FMH's sidebars the moment this line shipped. The check
  // is therefore an explicit === true against the flags the server sent, and
  // it is not routed through hasFeature() at all.
  if (pg === 'dailyclosing' &&
      !(window._featureFlags && window._featureFlags.daily_closing === true)) {
