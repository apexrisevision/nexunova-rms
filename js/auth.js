...
async function _completeLogin(user, company) {
  ...
  // Admin-visibility consent: add the revoke entry point, then force a decision
  // on first login for non-admins (blocks until answered). Fails open.
  _injectConsentMenuItem(user);
  await _checkAdminConsent();
...
/* ════════════════════════════════════════════════════════════
   ADMIN-VISIBILITY CONSENT
   The admin always keeps account control (password / rights / create-delete).
   But to SEE a user's dashboard data + activity, the user must opt in once
   (forced on first login). Revocable any time from the user menu.
   Backend: get_my_consent_status() / set_my_consent(bool).
════════════════════════════════════════════════════════════ */
const _consIcon = '<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>';
const _consChk  = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>';
const _consInfo = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';

function _ensureConsentStyles() {
  if (document.getElementById('nxn-consent-styles')) return;
  const st = document.createElement('style');
  st.id = 'nxn-consent-styles';
  st.textContent = `
  .nxnc-ov{position:fixed;inset:0;z-index:100000;display:flex;align-items:center;justify-content:center;background:rgba(7,11,24,.78);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);animation:nxncFade .25s ease;padding:20px}
  @keyframes nxncFade{from{opacity:0}to{opacity:1}}
  @keyframes nxncPop{from{opacity:0;transform:translateY(12px) scale(.97)}to{opacity:1;transform:none}}
  .nxnc-card{position:relative;width:100%;max-width:466px;background:#0F172A;border:1px solid rgba(255,255,255,.10);border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.55);overflow:hidden;animation:nxncPop .3s cubic-bezier(.2,.7,.2,1);color:#E2E8F0}
  [data-theme="light"] .nxnc-card{background:#fff;color:#0F172A;border-color:rgba(15,23,42,.10)}
  .nxnc-hd{padding:26px 24px 4px;text-align:center}
  .nxnc-ic{width:54px;height:54px;border-radius:14px;margin:0 auto 14px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#2563EB,#7C3AED);color:#fff}
  .nxnc-ttl{font-size:18px;font-weight:700;letter-spacing:-.01em;margin:0}
  .nxnc-sub{font-size:12.5px;opacity:.65;margin-top:5px}
  .nxnc-bd{padding:16px 24px 4px;font-size:13.5px;line-height:1.55}
  .nxnc-row{display:flex;gap:10px;padding:8px 0;align-items:flex-start}
  .nxnc-ok{color:#34D399;flex-shrink:0;margin-top:1px}
  .nxnc-no{color:#FBBF24;flex-shrink:0;margin-top:1px}
  .nxnc-note{margin:12px 0 0;padding:10px 12px;border-radius:10px;font-size:12px;line-height:1.5;background:rgba(37,99,235,.10);border:1px solid rgba(37,99,235,.22)}
  .nxnc-ft{display:flex;gap:10px;padding:18px 24px 22px}
  .nxnc-btn{flex:1;border:0;border-radius:11px;padding:12px 14px;font-size:14px;font-weight:600;cursor:pointer;transition:.15s}
  .nxnc-btn-no{background:rgba(148,163,184,.16);color:inherit}
  .nxnc-btn-no:hover{background:rgba(148,163,184,.26)}
  .nxnc-btn-yes{background:linear-gradient(135deg,#2563EB,#1D4ED8);color:#fff}
  .nxnc-btn-yes:hover{filter:brightness(1.08)}
  .nxnc-btn:disabled{opacity:.55;cursor:default}
  .nxnc-x{position:absolute;top:13px;right:15px;background:0;border:0;color:inherit;opacity:.5;cursor:pointer;font-size:22px;line-height:1;padding:2px 6px}
  .nxnc-x:hover{opacity:1}
  .nxnc-chip{display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;padding:4px 11px;border-radius:999px;margin-top:10px}
  .nxnc-chip.g{background:rgba(16,185,129,.15);color:#34D399}
  .nxnc-chip.d{background:rgba(245,158,11,.15);color:#FBBF24}`;
  document.head.appendChild(st);
}

// mode: 'decide' (forced, no dismiss) | 'manage' (dismissible). Resolves to
// 'granted' | 'declined' | null (manage dismissed without change).
function _consentModal({ mode, status, adminName, companyName }) {
  return new Promise(resolve => {
    _ensureConsentStyles();
    const old = document.getElementById('nxnc-ov'); if (old) old.remove();
    const _e = (s) => (typeof esc === 'function') ? esc(s || '') : (s || '');
    const admin = _e(adminName) || 'your administrator';
    const co    = _e(companyName);
    const manage = mode === 'manage';
    const chip = status === 'granted' ? '<span class="nxnc-chip g">● Currently sharing</span>'
               : status === 'declined' ? '<span class="nxnc-chip d">● Currently private</span>' : '';
    const ov = document.createElement('div');
    ov.className = 'nxnc-ov'; ov.id = 'nxnc-ov';
    ov.innerHTML = `
      <div class="nxnc-card" role="dialog" aria-modal="true">
        ${manage ? '<button class="nxnc-x" title="Close">&times;</button>' : ''}
        <div class="nxnc-hd">
          <div class="nxnc-ic">${_consIcon}</div>
          <h2 class="nxnc-ttl">Share with ${admin}?</h2>
          <div class="nxnc-sub">${co ? co + ' · ' : ''}Dashboard &amp; activity visibility</div>
          ${manage ? chip : ''}
        </div>
        <div class="nxnc-bd">
          <div class="nxnc-row"><span class="nxnc-ok">${_consChk}</span><span><b>${admin}</b> manages your account either way — password resets, your access rights, and your site assignment are unaffected by this choice.</span></div>
          <div class="nxnc-row"><span class="nxnc-ok">${_consChk}</span><span>If you <b>agree</b>, your dashboard, recovery records and daily activity become visible to ${admin} in their Command Center.</span></div>
          <div class="nxnc-row"><span class="nxnc-no">${_consInfo}</span><span>If you <b>decline</b>, your activity and your site's data stay private — visible only to you.</span></div>
          <p class="nxnc-note">You can change this any time from the user menu → <b>Data sharing</b>.</p>
        </div>
        <div class="nxnc-ft">
          <button class="nxnc-btn nxnc-btn-no"  data-v="0">${manage && status==='declined' ? 'Keep private' : 'No, keep private'}</button>
          <button class="nxnc-btn nxnc-btn-yes" data-v="1">${manage && status==='granted' ? 'Keep sharing' : 'Yes, I agree'}</button>
        </div>
      </div>`;
    ...
    if (manage) {
      ov.querySelector('.nxnc-x').onclick = () => done(null);
      ov.addEventListener('mousedown', (e) => { if (e.target === ov) done(null); });
    }
    ov.querySelectorAll('.nxnc-btn').forEach(btn => btn.onclick = async () => {
      const granted = btn.dataset.v === '1';
      ov.querySelectorAll('.nxnc-btn').forEach(b => b.disabled = true);
      try {
        const { data } = await supabase.rpc('set_my_consent', { p_granted: granted });
        if (data && data.success) {
          if (typeof notify !== 'undefined') notify.success(granted ? 'Sharing enabled' : 'Kept private', { duration: 2200 });
          done(granted ? 'granted' : 'declined');
        } else {
          if (typeof notify !== 'undefined') notify.error('Could not save', { detail: 'Please try again.' });
          ov.querySelectorAll('.nxnc-btn').forEach(b => b.disabled = false);
        }
      } catch (_) {
        if (typeof notify !== 'undefined') notify.error('Connection error', { detail: 'Please try again.' });
        ov.querySelectorAll('.nxnc-btn').forEach(b => b.disabled = false);
      }
    });
  });
...
// Force a decision on login for non-admins whose status is still 'pending'.
async function _checkAdminConsent() {
  try {
    const { data } = await supabase.rpc('get_my_consent_status');
    if (!data || !data.success || data.applicable === false) return;   // admins / no session → skip
    if (data.status === 'pending') {
      await _consentModal({ mode:'decide', status:'pending', adminName:data.admin_name, companyName:data.company_name });
    }
  } catch (_) { /* fail open — never block login on a consent hiccup */ }
}

// Manage/revoke entry point (from the sidebar user menu).
async function openConsentSettings() {
  let info = null;
  try { const { data } = await supabase.rpc('get_my_consent_status'); info = data; } catch (_) {}
  if (!info || !info.success || info.applicable === false) {
    if (typeof notify !== 'undefined') notify.success('Not applicable', { detail: 'Admins are not asked for data-sharing consent.' });
    return;
  }
  await _consentModal({ mode:'manage', status:info.status, adminName:info.admin_name, companyName:info.company_name });
}

// Add the "Data sharing" item to the sidebar user popover (non-admins only).
function _injectConsentMenuItem(user) {
  const role = (user && user.role || '').toLowerCase();
  if (role === 'owner' || role === 'admin') return;            // viewers, not subjects
  const pop = document.getElementById('sb-user-pop');
  if (!pop || document.getElementById('sb-pop-consent')) return;
  const btn = document.createElement('button');
  btn.className = 'sb-pop-item';
  btn.id = 'sb-pop-consent';
  btn.textContent = 'Data sharing';
  btn.onclick = () => { if (typeof _sbCloseUserMenu === 'function') _sbCloseUserMenu(); openConsentSettings(); };
  const div = pop.querySelector('.sb-pop-div');
  pop.insertBefore(btn, div || null);
}
